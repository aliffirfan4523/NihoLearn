...
export default async function GrammarPage({ params }: { params: Promise<{ level: string }> }) {
  ...
  const progress = await prisma.grammarProgress.findMany({ where: { userId: user.id, level: label } });
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 shadow-sm dark:border-white/20 dark:bg-[#1A1A1A]">
        <div className="pointer-events-none absolute -right-6 -top-12 font-serif text-[12rem] leading-none text-[#C84B31]/5 dark:text-[#C84B31]/10">文</div>
        <div className="relative">
          <p className="text-sm font-semibold uppercase tracking-widest text-[#C84B31] dark:text-[#E85C40]">JLPT {label}</p>
          <h2 className="mt-3 text-4xl font-bold tracking-tight text-[#1A1A1A] dark:text-[#FAFAFA]">{label} Grammar</h2>
          <p className="mt-4 text-[#6B6B6B] dark:text-[#A0A0A0]">{staticGrammar.length} grammar points. Click to expand examples and cycle status.</p>
        </div>
      </section>
