...
export default async function VocabularyPage({ params }: { params: Promise<{ level: string }> }) {
  ...
  // For now only N5 has static data
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 shadow-sm">
        <div className="pointer-events-none absolute -right-6 -top-12 font-serif text-[12rem] leading-none text-[#C84B31]/5">語</div>
        <div className="relative">
          <p className="text-sm font-semibold uppercase tracking-widest text-[#C84B31]">JLPT {label}</p>
          <h2 className="mt-3 text-4xl font-bold tracking-tight text-[#1A1A1A]">{label} Vocabulary</h2>
          <p className="mt-4 text-[#6B6B6B]">{staticVocab.length} words available. Click a status to cycle: unlearned → reviewing → mastered.</p>
        </div>
      </section>
