  ...
  return (
    <>
      <button
        ...
        className="fixed left-4 top-4 z-50 rounded-xl border border-black/10 bg-white p-3 text-[#1A1A1A] shadow-sm lg:hidden dark:border-white/20 dark:bg-[#2A2A2A] dark:text-[#FAFAFA]"
        aria-label="Toggle sidebar"
      >
        {sidebarOpen ? <X size={20} /> : <Menu size={20} />}
      </button>

      <aside className={`${sidebarOpen ? "translate-x-0 lg:w-72 lg:px-5" : "-translate-x-full lg:w-0 lg:translate-x-0 lg:px-0"} fixed inset-y-0 left-0 z-40 w-72 shrink-0 overflow-hidden border-r border-black/10 bg-white/95 px-5 py-6 shadow-xl transition-all duration-300 lg:static lg:shadow-none dark:border-white/10 dark:bg-[#1A1A1A]/95`}>
        <div className="flex items-center gap-3">
          <Link href="/" className="block flex-1 rounded-2xl bg-[#C84B31] px-4 py-4 text-white shadow-sm dark:bg-[#E85C40]">
          ...
          <button type="button" onClick={toggleSidebar} className="hidden rounded-xl border border-black/10 bg-[#FAFAF8] p-3 text-[#6B6B6B] lg:block dark:border-white/20 dark:bg-[#0A0A0A] dark:text-[#A0A0A0]" aria-label="Collapse sidebar">
            <X size={18} />
          </button>
        </div>
            ...
            <div className="space-y-4">
              {jlptLevels.map((level) => (
                <div key={level} className="rounded-2xl border border-black/5 bg-[#FAFAF8] p-3 dark:border-white/10 dark:bg-[#0A0A0A]">
                  <Link href={`/${level}`} className="font-semibold text-[#2D5F8A] hover:text-[#C84B31] dark:text-[#4A86B8] dark:hover:text-[#E85C40]">
                    {formatLevel(level)} Overview
                  </Link>
                  <div className="mt-2 space-y-1">
                    {levelSections.map((section) => (
                      <Link key={section} href={`/${level}/${section}`} className="block rounded-lg px-2 py-1 capitalize text-[#6B6B6B] hover:bg-white hover:text-[#C84B31] dark:text-[#A0A0A0] dark:hover:bg-[#2A2A2A] dark:hover:text-[#E85C40]">
                        {section}
                      </Link>
                    ))}
                  </div>
