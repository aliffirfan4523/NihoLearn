...
function KanaRowCard({ row, items, onSelect }: { row: string; items: KanaCharacter[]; onSelect: (item: KanaCharacter) => void }) {
  ...
  return (
    <article className="rounded-2xl border border-black/10 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-[#C84B31]">
      <div className="flex items-center gap-3">
        <h3 className="mr-auto text-sm font-semibold text-[#1A1A1A]">{row} row</h3>
        ...
        <span className="w-10 text-right text-xs font-semibold text-[#6B6B6B]">{percent}%</span>
      </div>

      <div className="mt-7 grid grid-cols-5 gap-3">
        {items.map((item) => (
          <button key={item.id} type="button" onClick={() => onSelect(item)} className="rounded-xl p-2 text-center transition hover:bg-[#FAFAF8] focus:outline-none focus:ring-2 focus:ring-[#C84B31]/40">
            <div className={`font-serif text-4xl font-bold leading-none ${mastered ? "text-[#3D7D52]" : "text-[#2D5F8A]"}`}>
              {item.character}
            </div>
            <div className="mt-3 font-mono text-xs text-[#6B6B6B]">{item.romaji}</div>
          </button>
        ))}
      </div>
    </article>
...
export function KanaRowSections({ kana, type }: { kana: KanaCharacter[]; type: "hiragana" | "katakana" }) {
  ...
  return (
    <div className="space-y-10">
      <header className="relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 shadow-sm">
        <div className="pointer-events-none absolute -right-5 -top-12 font-serif text-[12rem] leading-none text-[#C84B31]/5">
          {type === "hiragana" ? "あ" : "ア"}
        </div>
        <div className="relative">
          <h2 className="text-4xl font-bold tracking-tight text-[#1A1A1A]">{type === "hiragana" ? "Hiragana" : "Katakana"}</h2>
          <p className="mt-2 text-[#6B6B6B]">Average mastery: {average}%</p>
        </div>
      </header>
      ...
      {sectionList(type).map((section) => (
        <section key={section.title}>
          <div className="mb-5 flex items-center gap-3">
            ...
            <span className="text-sm text-[#6B6B6B]">{section.label}</span>
